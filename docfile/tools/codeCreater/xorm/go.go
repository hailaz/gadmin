...
func PKIsId(table *core.Table) bool {
	for _, col := range table.Columns() {
		if strings.ToLower(col.Name) == "id" && col.IsPrimaryKey {
			return true
		}
	}
	return false
...
func tag(table *core.Table, col *core.Column) string {
	...
	if len(res) > 0 {
		switch genOrmType {
		case "gorm":
			tags = append(tags, "gorm:\""+strings.ToLower(strings.Join(res, ";"))+"\"")
		case "xorm":
	...
	if len(tags) > 0 {
		return "`" + strings.Join(tags, " ") + "`" + "//" + comment
	} else {
		return ""
	}
}
